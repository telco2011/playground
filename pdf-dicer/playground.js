...
dicer
    ...
    .on('pagesAnalyzed', pages => {
      ...
      for (var key in range) {
        if (range.hasOwnProperty(key)) {
          ...
          scissors(path.join(__dirname + '/input/example-alternating.pdf'))
            .range(from, to)
            .pdfStream().pipe(fs.createWriteStream(`${outputPath}/range-${key}.pdf`))
    ...
    .split(__dirname + '/input/example-alternating.pdf', options, function(err, output) {
